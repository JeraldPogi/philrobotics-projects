...
//
//----------------------------------------------------------------------------------
// Filename:	hal_Anito_gpio.c 
// Description:	General Purpose Input/Output Hardware Abstraction Layer for PIC
// Revision:    v01.00.00
...
void configPin(unsigned int8_t ui8PinName, bool_t blDirection)
{
...
bool_t checkPinState(unsigned int8_t ui8PinName)
{
	if(ui8PinName<=D7){
...
void togglePin(unsigned int8_t ui8PinName)
{
    if(ui8PinName<=D7) { 
...
void setPinState(unsigned int8_t ui8PinName, bool_t blDirection)
{
    if(blDirection){
        if(ui8PinName<=D7) { 
            REGISTER_PORTC |= (1UL<<ui8PinName); 
