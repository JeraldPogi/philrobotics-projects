...
extern int ucunit_checkpoints[];            /* Max. number of tracepoints */
...
/**
 * @Macro:       UCUNIT_CheckIsNotEqual(expected,actual)
 *
 * @Description: Checks that actual value is not equal the expected value.
 *
 * @Param expected: Expected value.
 * @Param actual: Actual value.
 *
 * @Remarks:     This macro uses UCUNIT_Check(condition, msg, args).
 *                  Added by glutnix_neo: 13-May-2012
 */
...
/**
 * @Macro:       UCUNIT_CheckTracepointNonCoverage(index)
 *
 * @Description: Checks if a trace point was not covered.
 *
 * @Param index: Index of the trace point.
 *
 * @Remarks:     This macro fails if index>UCUNIT_MAX_TRACEPOINTS.
 *                  Added by glutnix_neo: 13-May-2012
 */
#define UCUNIT_CheckTracepointNonCoverage(index)    \
    UCUNIT_Check( (ucunit_checkpoints[index]==0), "TracepointNonCoverage", #index);
